<script>
    import Entete from '../../../components/entete.svelte';
    import BoutonSoumettre from '../../../components/boutonSoumettre.svelte';
    import '/src/app.css';
    export let data;
    const role = data.role;

</script>

<Entete />
<div class="login-container">
    <div class="login-box">
    <h2>Ajouter un nouveau rôle</h2>
    <p>Contactez votre administrateur système pour attribuer des accès à ce rôle après sa création!</p>

    <form method="POST" action="?/new">
    <div class="input-group">
    <label for="nom">Nom du rôle:</label>
    <input type="text" name="nom" required><br><br>
    </div>

    <div class="input-group">
    <label for="description">Description du rôle:</label>
    <textarea name="description"  cols="38" rows="10" required></textarea>
    </div>
    <div class="centrer">
        <BoutonSoumettre texte={"Enregistrer"}/>
        <a href = "/roles" ><button>Annuler</button></a>
    </div>
    </form>
    </div>
</div>

<style>
    .centrer, h2{
        text-align: center;
    }

    button {
      padding: 0.75rem;
      border: none;
      border-radius: 4px;
      background-color: gray;
      color: white;
      font-size: 1rem;
      cursor: pointer;
      text-align: center;
    }
  
    button:hover {
      background-color: #695C4B;
    }

    .centrer{
        text-align: center;
    }
</style>